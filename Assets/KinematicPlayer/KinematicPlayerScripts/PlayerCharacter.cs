...
public class PlayerCharacter : MonoBehaviour, ICharacterController
{
    ...
    [Range(0f, 1f)]
    [SerializeField] private float wallRunVerticalMomentumPreservation = 0.3f;
    ...
    {
        _crouchMode = holdToCrouch ? CrouchMode.Hold : CrouchMode.Toggle;
    ...
    public void UpdateInput(CharacterInput input)
    {
        ...
        _requestedMovement = Vector3.ClampMagnitude(input.Rotation * new Vector3(input.Move.x, 0f, input.Move.y), 1f);
        ...
        _requestedSustainedJump = isSustainedJumpEnabled && input.JumpSustain;
    ...
    public void UpdateVelocity(ref Vector3 currentVelocity, float deltaTime)
    {
        ...
        if (_wallJumpInputLockoutTimer > 0f) { _wallJumpInputLockoutTimer -= deltaTime; }
    ...
    private void HandleGroundedMovement(ref Vector3 currentVelocity, float deltaTime)
    {
        ...
        if (_state.Stance == Stance.WallRun) { _state.Stance = Stance.Stand; }

        var groundedMovement = motor.GetDirectionTangentToSurface(
            _requestedMovement, motor.GroundingStatus.GroundNormal
        ) * _requestedMovement.magnitude;

        bool isTryingToSlide = _state.Stance == Stance.Crouch && (_lastState.Stance == Stance.Stand || !_lastState.Grounded);
    ...
    {
        if (!_requestedJump || _jumpCooldownTimer > 0f) { return; }
        ...
        else
        {
            ...
            if (_timeSinceJumpRequest >= coyoteTime) { _requestedJump = false; }
        }
    }
    ...
    private void HandleStandardAirMovement(ref Vector3 currentVelocity, float deltaTime)
    {
        ...
        {
            if (!motor.GroundingStatus.IsStableOnGround) { movementForce = Vector3.ProjectOnPlane(movementForce, motor.GroundingStatus.GroundNormal); }
        }
    ...
    private void ApplyGravity(ref Vector3 currentVelocity, float deltaTime)
    {
        ...
        if (_requestedSustainedJump && Vector3.Dot(currentVelocity, motor.CharacterUp) > 0f) { effectiveGravity *= jumpSustainGravity; }
    ...
    private void HandleSlideMovement(ref Vector3 currentVelocity, float deltaTime, bool isStartingSlide)
    {
        ...
        else
        {
            ...
            if (currentSpeed < slideEndSpeed) { _state.Stance = Stance.Crouch; }
        }
    }

    public void PostGroundingUpdate(float deltaTime)
    {
        if (!motor.GroundingStatus.IsStableOnGround && _state.Stance == Stance.Slide) { _state.Stance = Stance.Crouch; }
        if (motor.GroundingStatus.IsStableOnGround && _state.Stance == Stance.WallRun) { _state.Stance = Stance.Stand; }
    }
    ...
    private void CheckForDirectionalWallJump()
    {
        ...
        if (Physics.SphereCast(origin, sphereRadius, direction, out RaycastHit hit, wallJumpCastDistance, motor.CollidableLayers, QueryTriggerInteraction.Ignore))
        {
    ...
    private void CheckForWallRun()
    {
        ...
        _isWallOnLeft = Physics.SphereCast(origin, sphereRadius, -transform.right, out RaycastHit leftHit, wallRunCheckDistance, motor.CollidableLayers, QueryTriggerInteraction.Ignore);
        if (_isWallOnLeft) { _leftWallNormal = leftHit.normal; }

        _isWallOnRight = Physics.SphereCast(origin, sphereRadius, transform.right, out RaycastHit rightHit, wallRunCheckDistance, motor.CollidableLayers, QueryTriggerInteraction.Ignore);
        if (_isWallOnRight) { _rightWallNormal = rightHit.normal; }
    }
    ...
    public void UpdateBody(float deltaTime)
    {
        ...
        cameraTarget.localPosition = Vector3.Lerp(cameraTarget.localPosition, new Vector3(0f, cameraTargetHeight, 0f), lerpFactor);
    ...
    public CrouchMode GetCrouchMode() => _crouchMode;
